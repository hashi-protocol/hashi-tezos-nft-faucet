...
def test():
    ...
    cyber_token = Cyber_Token(FA2_config(single_asset=False,non_fungible=True), admin = admin.address, metadata = sp.big_map({"": sp.utils.bytes_of_string("tezos-storage:content"),"content": sp.utils.bytes_of_string("""{"name" : "Hashi NFT"}""")}))
    ...
    scenario += cyber_token.burn(address = mark.address, token_id = 0, eth_owner = sp.utils.bytes_of_string('0xadresseth')).run(sender = mark)
    ...
    scenario += locker.unlockToken(0).run(sender=elon)
    scenario += locker.withdraw(0).run(sender=elon)
